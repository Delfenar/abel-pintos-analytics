...
export const InstagramView: React.FC = () => {
  ...
  const instagramContent = React.useMemo(() => {
    const liveItems = getLatestSnapshotPerContent(liveSheetsRecords.filter(r => r.plataforma === 'Instagram'));
    if (liveItems.length > 0) {
      return liveItems.map(r => ({
        ...
        metrics: {
          ...
          engagementRate: cleanNumber(r.metricas.alcance) > 0 ? Number(((cleanNumber(r.metricas.interacciones) / cleanNumber(r.metricas.alcance)) * 100).toFixed(2)) : 5.8,
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="h-60 w-full flex items-center justify-center">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#0F172A', borderColor: '#E1306C', borderRadius: '0.75rem', fontSize: '12px', color: '#FFFFFF' }} 
      ...
      <ContentTable title="Publicaciones Destacadas en @abelpintos (Google Sheets)" items={instagramContent} />
    </div>
  );
};
